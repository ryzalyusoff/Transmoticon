...
$(document).ready(function(){
	...
	$(document).bind("mouseup",function(event){
		var keyword = String(getSelected());
		$('#pop').css("display","none");
		$.post('http://fikhrie.net/transmoticon/',{q : keyword }, function(receive) {
			//console.log(receive);
			/*if ( receive.image != "" ){
				receive.image = "<img src='" + receive.image + "' />";
			}*/
			$('#pop').css("display","block").css("webkit-border-radius","5px").css("font-family","arial,sans-serif").css("background-color","#eeeeee").css("font-size","12px").css("padding","10px").css("border","1px solid #727272").css("top",event.pageY).css("left",event.pageX).html(
				"<div style='width:240px;height:25px;border-bottom:1px solid #727272;margin-bottom:10px;' >" + receive.image + " " + receive.meaning + "</div>"
